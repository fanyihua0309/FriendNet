...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            grid_x = torch.linspace(0, input_width - 1, input_width).repeat(input_height, 1).repeat(
                batch_size * len(self.anchors_mask[i]), 1, 1).view(x.shape).type(FloatTensor).to(device)
            grid_y = torch.linspace(0, input_height - 1, input_height).repeat(input_width, 1).t().repeat(
                batch_size * len(self.anchors_mask[i]), 1, 1).view(y.shape).type(FloatTensor).to(device)

            # ----------------------------------------------------------#
            #   按照网格格式生成先验框的宽高
            #   batch_size,3,20,20
            # ----------------------------------------------------------#
            anchor_w = FloatTensor(scaled_anchors).index_select(1, LongTensor([0])).to(device)
            anchor_h = FloatTensor(scaled_anchors).index_select(1, LongTensor([1])).to(device)
            ...
            #   将输出结果归一化成小数的形式
            # ----------------------------------------------------------#
            _scale = torch.Tensor([input_width, input_height, input_width, input_height]).type(FloatTensor).to(device)
            output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale,
                                conf.view(batch_size, -1, 1), pred_cls.view(batch_size, -1, self.num_classes)), -1)
            outputs.append(output.data)
        return outputs

    def yolo_correct_boxes_np(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        # -----------------------------------------------------------------#
    ...
    def non_max_suppression_np(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5,
                            nms_thres=0.4):
        # ----------------------------------------------------------#
        ...
        for i, image_pred in enumerate(prediction):
            ...
            if output[i] is not None:
                ...
                output[i][:, :4] = self.yolo_correct_boxes_np(box_xy, box_wh, input_shape, image_shape, letterbox_image)
        return output

    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        # Swap x and y dimensions for torch tensor
        box_yx = box_xy.flip(-1)
        box_hw = box_wh.flip(-1)

        input_shape = torch.tensor(input_shape, dtype=torch.float32).cuda()
        image_shape = torch.tensor(image_shape, dtype=torch.float32).cuda()
